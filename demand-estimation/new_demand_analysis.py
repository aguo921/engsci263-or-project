...
def write_demand_to_file(demand_dict):
    ...
    for key in demand_dict:
        ...
        weekday_dict[key] = weekday
        sat_dict[key] = sat
        weekday_upper = np.percentile(weekday, 75)
        sat_upper = np.percentile(sat, 75)
        ...
        store_demandP.write(f"{brand},{location},{weekday_upper},{sat_upper}\n")
        store_demand.write(f"{brand},{location},{math.ceil(mean_wd)},{math.ceil(mean_sat)}\n")
    store_demandP.close()
    store_demand.close()
    return weekday_dict, sat_dict


def plot_demand_frequency(reference_dict, supermarket="Four Square"):
    for key in reference_dict:
        if supermarket in key:
            prob_plot = sns.displot(reference_dict[key], discrete=True, kde=True, height=10)
            prob_plot.fig.subplots_adjust(top=.95, bottom=.10)
            prob_plot.ax.set_title(f"Demand Frequency of {key} on weekdays")
            prob_plot.set(xlabel="pallet demanded")
    plt.show()
...
def bootstrap_weekday_demand(reference_dict):
    ...
    for i in range(1,1001):
        heading += f"Run{i},"
    wd.write(heading[:-1]+'\n')
    weekday_df = pd.DataFrame.from_dict(reference_dict)
    for key in reference_dict:
        data = weekday_df[key]
        wd.write(f"{key},")
        string = ""
        sampled = data.sample(n=1000, replace=True)
        for val in sampled:
            string += str(val) + ","
        string = string[:-1] + "\n"
        wd.write(string)
...
if __name__ == "__main__":
    ...
    weekday, sat = write_demand_to_file(demand_dict)
    bootstrap_weekday_demand(weekday)
    bootstrap_sat_demand(sat)